...
def generate_outfits_from_selected_garment(
    ...
    def maybe_add_extras(base: List[Garment], top_item: Optional[Garment] = None):
        ...
        if (
            ...
            and not (has_one_piece and occasion in ["matrimonio", "gala", "salida nocturna", "cita"] and not matrimonio_midlayer_allowed)
        ):
            for mid in top_candidates["midlayer"][:3]:
                combo_mid = base + [mid]
                add_combo(combo_mid)
            ...
            for bottom in top_candidates["bottom"]:
                if occasion in ["matrimonio", "gala"] or (occasion == "cita" and mood == "elegante"):
                    if not (occasion == "matrimonio" and mood == "urbano"):
                        ...
                        if not (garment_has_style(bottom, "elegante") or garment_has_style(bottom, "formal") or garment_has_style(bottom, "urbano")):
                            continue
